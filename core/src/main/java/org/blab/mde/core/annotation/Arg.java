...
/**
 * Parameters that are annotated with this annotation will be assigned the value of the parameter of
 * the source method with the given parameter. For example, if source method {@code foo(String,
 * Integer)} is bound to target method {@code bar(@Arg(1) Integer)}, the second parameter of {@code
 * foo} will be bound to the first argument of {@code bar}. <p>&nbsp;</p> If a source method has
 * less parameters than specified by {@link Arg#value()}, the method carrying this parameter
 * annotation is excluded from the list of possible binding candidates to this particular source
 * method. The same happens, if the source method parameter at the specified index is not assignable
...
public @interface Arg {
  ...
  /**
   * Determines if the argument binding is to be considered by a {@link
   * net.bytebuddy.implementation.bind.ArgumentTypeResolver} for resolving ambiguous bindings of two
   * methods. If {@link net.bytebuddy.implementation.bind.annotation.Argument.BindingMechanic#UNIQUE},
   * of two bindable target methods such as for example {@code foo(String)} and {@code bar(Object)},
   * the {@code foo} method would be considered as dominant over the {@code bar} method because of
   * its more specific argument type. As a side effect, only one parameter of any target method can
   * be bound to a source method parameter with a given index unless the {@link
   * net.bytebuddy.implementation.bind.annotation.Argument.BindingMechanic#ANONYMOUS} option is used
  ...
  enum BindingMechanic {
    ...
    /**
     * The binding is unique, i.e. only one such binding must be present among all parameters of a
     * method. As a consequence, the binding can be latter identified by an {@link
     ...
     * @param typing               Indicates if dynamic type castings should be attempted for
   ...
   */
  enum Binder implements TargetMethodAnnotationDrivenBinder.ParameterBinder<Arg> {
    ...
    @Override
    public Class<Arg> getHandledType() {
      return Arg.class;
    }

    @Override
    public MethodDelegationBinder.ParameterBinding<?> bind(AnnotationDescription.Loadable<Arg> annotation,
                                                           MethodDescription source,
                                                           ParameterDescription target,
                                                           Implementation.Target implementationTarget,
                                                           Assigner assigner,
                                                           Assigner.Typing typing) {
      Arg argument = annotation.loadSilent();
      if (argument.value() < 0) {
        throw new IllegalArgumentException("@Arg annotation on " + target + " specifies negative index");
